...
function obtenerDestinatarios($pdo_users) {
    $stmt = $pdo_users->prepare("SELECT `correo` FROM users WHERE notificar = 1");
    $stmt->execute();
    $correos = [];
...
foreach ($rows as $row) {
    ...
    if (!empty($cambiosParaCorreo)) {
        ...
        if (!empty($destinatarios)) {
            ...
            enviarCorreoCambioCarga($nombreRegistro, $idValue, $cambiosParaCorreo, $usuarioBitacora, $fechaBitacora, $destinatarios, $pdo);
        }
    }
}
